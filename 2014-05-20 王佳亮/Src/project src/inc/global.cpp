...
bool StopPointex::operator ==(const StopPointex& p) const
{
	const StopPoint& rthis = *this;
	const StopPoint& rp = p;
	if( rthis == rp && CID == p.CID && State == p.State && ID == p.ID && 
		fabs(ReachDis - p.ReachDis) < FLOAT_EQUAL && fabs(CoreDis - p.CoreDis) < FLOAT_EQUAL )
	{
		return true;
	}
...
{
	/*double radLat1 = DegToRad(p1.Lat);
	double radLat2 = DegToRad(p2.Lat);
	double a = radLat1 - radLat2;
	double b = DegToRad(p1.Lng) - DegToRad(p2.Lng);
...
double CalAverPointDis(double a, int n)
{
	return (sqrt(a) / sqrt(n - 1));
}